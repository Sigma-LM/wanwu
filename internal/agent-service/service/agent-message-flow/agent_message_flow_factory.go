...
func NewAgentMessageFlow(multiAgent bool) *compose.Graph[*request.AgentChatContext, []*schema.Message] {
	//创建图信息
	...
	//增加图节点
	AddAgentFlowNode(graph, multiAgent)
	//连接图信息
	BuildGraphEdge(graph)
	return graph
}

func AddAgentFlowNode(graph *compose.Graph[*request.AgentChatContext, []*schema.Message], multiAgent bool) {
	...
	if multiAgent {
		_ = graph.AddChatTemplateNode(keyOfPromptTemplate, nodes.MultiChatPrompt)
	} else {
