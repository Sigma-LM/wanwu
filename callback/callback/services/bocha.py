...
class BochaMultimodalSearch:
    ...
    def _search_internal(self, **kwargs) -> BochaResponse:
        ...
        try:
            ...
            response = requests.post(self.BOCHA_BASE_URL, headers=header, json=payload, timeout=30)
    ...
    def comprehensive_search(self,api_key:str, query: str, max_results: int = 10) -> BochaResponse:
        """
    ...
    def web_search_only(self, api_key:str,query: str, max_results: int = 15) -> BochaResponse:
        """
    ...
    def search_last_24_hours(self, api_key:str,query: str) -> BochaResponse:
        ...
        return self._search_internal(header=get_bocha_header(api_key),query=query, freshness='oneDay', answer=True)
    ...
    def search_last_week(self, api_key:str,query: str) -> BochaResponse:
        ...
        return self._search_internal(header=get_bocha_header(api_key),query=query, freshness='oneWeek', answer=True)
...
if __name__ == "__main__":
    ...
    try:
        ...
        response1 = search_client.comprehensive_search(api_key=api_key,query="人工智能对未来教育的影响")
        print_response_summary(response1)

        # 场景2: Agent需要查询特定结构化信息 - 天气
        if isinstance(search_client, BochaMultimodalSearch):
            response2 = search_client.search_for_structured_data(api_key=api_key,query="上海明天天气怎么样")
        ...
        if isinstance(search_client, BochaMultimodalSearch):
            response3 = search_client.search_for_structured_data(api_key=api_key,query="东方财富股票")
            print_response_summary(response3)

        # 场景4: Agent需要追踪某个事件的最新进展
        response4 = search_client.search_last_24_hours(api_key=api_key,query="C929大飞机最新消息")
        print_response_summary(response4)

        # 场景5: Agent只需要快速获取网页信息，不需要AI总结
        if isinstance(search_client, BochaMultimodalSearch):
            response5 = search_client.web_search_only(api_key=api_key,query="Python dataclasses用法")
            print_response_summary(response5)

        # 场景6: Agent需要回顾一周内关于某项技术的新闻
        response6 = search_client.search_last_week(api_key=api_key,query="量子计算商业化")
